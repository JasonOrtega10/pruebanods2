...
const configVariables = [
    ...
    },
    {
        ...
        description: "Message which gets sent to the share recipients. {creator} and {shareUrl} will be replaced with the creator's name and the share URL.",
        type: "text",
    ...
    },
    {
        ...
        description: "Subject of the email which gets sent when someone created a share with your reverse share link.",
        type: "string",
    ...
    },
    {
        ...
        description: "Message which gets sent when someone created a share with your reverse share link. {shareUrl} will be replaced with the creator's name and the share URL.",
        type: "text",
    ...
    },
    {
        ...
        description: "Message which gets sent when a user requests a password reset. {url} will be replaced with the reset password URL.",
        type: "text",
...
async function main() {
    ...
    for (const configVariableFromDatabase of configVariablesFromDatabase) {
        const configVariable = configVariables.find((v) => v.key == configVariableFromDatabase.key);
        if (!configVariable) {
            await prisma.config.delete({
                where: { key: configVariableFromDatabase.key },
            });
        }
        else if (JSON.stringify(Object.assign(Object.assign({}, configVariable), { key: configVariableFromDatabase.key, value: configVariableFromDatabase.value })) != JSON.stringify(configVariableFromDatabase)) {
            await prisma.config.update({
                where: { key: configVariableFromDatabase.key },
                data: Object.assign(Object.assign({}, configVariable), { key: configVariableFromDatabase.key, value: configVariableFromDatabase.value }),
            });
        }
    }
}
