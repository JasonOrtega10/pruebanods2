...
import { FileStatus, FileUploadResponse } from "../types/File.type";
...
const create = async (share: CreateShare) => {
  const { id, expiration, recipients, security, description, allowDownload, showEstado, referencia} = share;
  return (
...
const setFileStatus = async (
  ...
  await api.put(`shares/${shareId}/files/${fileId}`, null, {
    params: {
      status,
    },
  });
